...
const Router = () => {
  const route = (routeId: string, event: MouseEvent<HTMLButtonElement>) => {
    if (routeId === ROUTE_ID_SINGLE_MATCH) {
      ...
      root.render(
        <React.StrictMode><ConfigureTournament /></React.StrictMode>
      );
    }
  };
  ...
  return (
    <div className='router-container'>
      <button onClick={(event) => route(ROUTE_ID_SINGLE_MATCH, event)}>Single Match</button>
      <button onClick={(event) => route(ROUTE_ID_TOURNAMENT, event)}>Tournament</button>
    </div>
  );
}

...
const DisplayAllGroups = (props: any) => {
  const standingDomElements: JSX.Element[] = [];
  if (props.groupStanding) {
    let standingGroupName: keyof typeof props.groupStanding;
    for (standingGroupName in props.groupStanding) {
      standingDomElements.push(
        <div key={standingGroupName} className="group">
          ...
          <div className="group-data">
            ...
            {props.groupStanding[standingGroupName].map(
              (teamStanding: TeamStanding, index: number) => {
                return (
                  <div key={index} className="group-data-content">
                    <div>{teamStanding.team.name}</div>
  ...
  if (props.groupMatches) {
    let matchGroupName: keyof typeof props.groupMatches;
    for (matchGroupName in props.groupMatches) {
      matchDomElements.push(
        <div key={matchGroupName} className="group">
          ...
          <div className="round">
            {props.groupMatches[matchGroupName].map(
                ...
                return (
                  <ul
                    ...
                    onClick={() =>
                      props.startMatch(
                        match,
                        props.groupNoDeuce || false,
                        props.groupScoreToWin || GAME_DEFAULT_SCORE_TO_WIN
                      )
                    }
                  >
                    <li
...
const TournamentFixture = (props: TournamentConfig) => {
  const [screenToDisplay, setScreenToDisplay] = useState(SCREEN_FIXTURE);
  const [match, setMatch] = useState({} as Match);
  const [noDeuce, setNoDeuce] = useState(false);
  const [scoreToWin, setScoreToWin] = useState(GAME_DEFAULT_SCORE_TO_WIN);
  const [tournamentConfig, setTournamentConfig] = useState(props);

  const startMatch = (match: Match, noDeuce: boolean, scoreToWin: number) => {
    if (!match.winner) {
      if (
        window.confirm(
          `Start match between [${match.teams[0].name}] and [${match.teams[1].name}]?`
        )
      ) {
        setMatch(match);
        setNoDeuce(noDeuce);
        setScoreToWin(scoreToWin);
        setScreenToDisplay(SCREEN_MATCH);
      }
    }
  };
  ...
  useEffect(() => {
    if (screenToDisplay === SCREEN_FIXTURE) {
      axios
        .get(`${serverUrl}/tournament/getActiveTournament`)
        .then((response) => {
          if (response.data.config) {
            setTournamentConfig(JSON.parse(response.data.config));
          }
        });
    }
  }, [screenToDisplay]);
  ...
  return (
    <>
      {screenToDisplay === SCREEN_MATCH && (
        <PlayMatch
          matchId={match.matchId || ""}
          servingPlayer1={match.teams[0].player1 || ""}
          servingPlayer2={match.teams[0].player2 || ""}
          receivingPlayer1={match.teams[1].player1 || ""}
          receivingPlayer2={match.teams[1].player2 || ""}
          noDeuce={noDeuce}
          scoreToWin={scoreToWin}
          isTournamentMatch={true}
          propagateTo={showFixture}
        />
      )}
      {screenToDisplay === SCREEN_FIXTURE && (
        <div>
          <details className="group-stage" open>
            <summary className="group-stage-summary">Group Stage</summary>
            <DisplayAllGroups
              groupMatches={tournamentConfig.groupMatches}
              groupStanding={tournamentConfig.groupStanding}
              groupNoDeuce={tournamentConfig.groupNoDeuce}
              groupScoreToWin={tournamentConfig.groupScoreToWin}
